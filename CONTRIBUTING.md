...
We use [pytest](https://docs.pytest.org) to test our code base. If your contribution introduces new components, you should write new tests to make sure your code doesn't crash under normal circumstances. After installing the test dependencies (`pytest` and `pytest-cov`), add the tests to the [tests/](tests) directory and run them with

```
pytest --cov lampe
```

It is also recommended to ensure your code works as expected within toy experiments similar to the [tutorials](tutorials).
...
* Avoid small commits that revert/fix something introduced in the previous one. Remember `git commit --amend` is your best friend.
* Consider [starting commit messages with an emoji](https://gitmoji.dev) to depict the intention of commits.
* Have fun!
